...
public class main {
    ...
    public static void distributed_tracking(EndpointTree t) throws SQLException {
        ...
        float v_e=0;
        m=s.count("Query");
        while(s.count("Query")>1&&s.count("Element")>0){    //program loop while there are queries
                    ...
                    while(index>0){ //endpoint tree binary seach
                        if(s.selectFieldWhere("RangeLower", "Node", "Ind", String.valueOf(index))<=v_e){
                            ...
                            if(s.selectFieldWhere("RangeLower", "Node", "Ind", 
                                ...
                                index=(int)s.selectFieldWhere("RightChild", "Node", "Ind", String.valueOf(index));
                            }
                            else{
                                index=(int)s.selectFieldWhere("LeftChild", "Node", "Ind", String.valueOf(index));
                            }
                        }else{
                            break;
                        }
    ...
    public static void load_reset(){
        ...
        System.out.println("LOADING ELEMENTS/DATASTREAM ... ");
        s.tryUpdate("DELETE FROM Query WHERE QueryId >0;");
        System.out.println("LOADING QUERIES ...");
        fill("elements.txt","test");
    }
    
    public static void fill(String f1, String f2){  //insert into DB from file input
        String values;
        try{
    ...
    public static void main(String[] args) throws SQLException {
        ...
        //s=new SQL(args[0], args[1], args[2],args[3], args[4]);    //I4getCMD_args...parseString(args[n])
